/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { LectureWhereUniqueInput } from "./LectureWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { LectureUpdateInput } from "./LectureUpdateInput";

@ArgsType()
class UpdateLectureArgs {
  @ApiProperty({
    required: true,
    type: () => LectureWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => LectureWhereUniqueInput)
  @Field(() => LectureWhereUniqueInput, { nullable: false })
  where!: LectureWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => LectureUpdateInput,
  })
  @ValidateNested()
  @Type(() => LectureUpdateInput)
  @Field(() => LectureUpdateInput, { nullable: false })
  data!: LectureUpdateInput;
}

export { UpdateLectureArgs as UpdateLectureArgs };
